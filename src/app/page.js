...
const page = () => {
  ...
  const [currentImage, setCurrentImage] = useState()
	...
	const { addToCart, cartItems, removeFromCart, deleteCartItem } = useContext(ShopContext);
					...
					<div className={styles.mobileSlide}>
						{products.map((item, index) => {
							return (
								<div
									className={
										slideIndex === index + 1 ? styles.mobileImageToggle : styles.hideImageToggle
									}
									key={index}
								>
									<img
										...
										onClick={() => {setShowLightBox(true); setCurrentImage(item.mainImage)
											// setValue(index)
                    }}
									/>
								...
								return (
									...
									>
										<img
											src={item.thumbnail}
											alt={item.alt}
											className={index === value ? styles.selected : styles.default}
										/>
									</li>
								);
							})}
